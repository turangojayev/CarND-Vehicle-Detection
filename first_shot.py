...
def train(X, y):
    ...
    X_train = numpy.array(list(map(preprocess, X_train)))  # .reshape(*X_train.shape[:-1], 1)
    X_test = numpy.array(list(map(preprocess, X_test)))  # .reshape(*X_test.shape[:-1], 1)
    print(X_train.shape)
    pipeline = make_pipeline(
        PCA(n_components=800, whiten=True),
...
if __name__ == '__main__':
    ...
    # pix_per_cell = 8
    # cell_per_block = 2
    # normalized = contrast_normalization(gray)
    # print(normalized.max())
    # features, hog_image = get_hog_features(normalized, orient,
